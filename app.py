...
def analyze_tissue_coherence(df, product, date_filter=None):
    ...
    for idx, city_row in city_analysis.iterrows():
        ...
        if len(city_bricks) > 0:
            ...
            total_pf_quantiles = df_filtered[cols['pf']].quantile([0.3, 0.5, 0.7])
            
            # Strateji belirleme
            if pazar_payı_segment < 30 and pazar_büyüklüğü < total_pf_quantiles.get(0.5, 0):
            ...
            elif pazar_büyüklüğü < total_pf_quantiles.get(0.3, 0) and high_perf_ratio > 50:
                yatırım_stratejisi = "💎 Potansiyel"
                strateji_aciklama = "Küçük ama verimliliği yüksek niş bölgeler."
            else:
                yatırım_stratejisi = "👁️ İzleme"
                strateji_aciklama = "Düşük performanslı ve önceliği dşk yerler."
    ...
    if not brick_bcg_df.empty and 'Brick' in brick_bcg_df.columns:
        ...
        for city_name in results_df['Şehir'].unique():
            # Şehirdeki brick'leri bul
            city_brick_names = brick_analysis[brick_analysis['City'] == city_name]['Brick'].unique()
            
            # Bu brick'lerin BCG bilgilerini al
            city_bricks_in_bcg = brick_bcg_df[brick_bcg_df['Brick'].isin(city_brick_names)]
            ...
            if not city_bricks_in_bcg.empty:
                ...
                if len(most_common_bcg) > 0:
                    ...
                    # BCG yoksa, şehir performansına göre belirle
                    city_perf = results_df[results_df['Şehir'] == city_name]['Şehir_Performans'].iloc[0] if not results_df[results_df['Şehir'] == city_name].empty else 'Orta'
                    if city_perf == 'Yüksek':
                        city_bcg_mapping[city_name] = "⭐ Star"
                    elif city_perf == 'Orta':
                        city_bcg_mapping[city_name] = "❓ Question Mark"
                    else:
                        city_bcg_mapping[city_name] = "🐶 Dog"
            else:
                # BCG yoksa, şehir performansına göre belirle
                city_perf = results_df[results_df['Şehir'] == city_name]['Şehir_Performans'].iloc[0] if not results_df[results_df['Şehir'] == city_name].empty else 'Orta'
                if city_perf == 'Yüksek':
